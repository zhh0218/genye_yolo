...
mi = m.index("class CoordAttV2(nn.Module):")
mj = m.index("class RGBDCoordAttV2(nn.Module):")
main_cls = m[mi:mj]
assert 'gate_mode in ("spatial", "spatial_channel"' in main_cls, "main CoordAttV2 missing spatial branch"
gi = g.index("class CoordAttV2(nn.Module):")
gj = g.index("class RGBDCoordAttV2(nn.Module):")
...
assert g.count(a) == 2, f"E2 expected 2 got {g.count(a)}"
g = g.replace(a, b)
...
assert g.count(a2) == 2, f"E3 expected 2 got {g.count(a2)}"
g = g.replace(a2, b2)

# E4: build_model _fusion_kw add gate_mode from yaml
a3 = "_fusion_kw = dict(adaptive_gate=_adaptive_gate) if _adaptive_gate else {}"
b3 = (
    "_fusion_kw = dict(adaptive_gate=_adaptive_gate, gate_mode=d.get('gate_mode', 'channel')) if _adaptive_gate else {}"
)
assert g.count(a3) == 1, f"E4 expected 1 got {g.count(a3)}"
g = g.replace(a3, b3)
...
y = y.replace(
    "modality_adaptive_gate: False # learned modality-priority gate: α·main + (1−α)·aux + attn_out",
...
open(DSTY, "w", encoding="utf-8").write(y)
print("gated yaml ->", DSTY)
...
t = t.replace(
    'model_yaml_path = r"/workspace/ultralytics-main_for_genye/ultralytics/cfg/models/11/yolo11-seg.yaml"',
...
open(DSTT, "w", encoding="utf-8").write(t)
print("gated train script ->", DSTT)
print("ALL DONE")
