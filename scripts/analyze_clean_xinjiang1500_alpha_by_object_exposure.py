...
OUT_JSON = OUT_DIR / "clean_xinjiang1500_best_val300_alpha_by_object_exposure.json"
OUT_CSV = OUT_DIR / "clean_xinjiang1500_best_val300_alpha_by_object_exposure.csv"
...
class AlphaExposureValidator(SegmentationValidator):
    ...
    def update_metrics(self, preds, batch):
        img = batch["img"].detach().float()  # [B,3,H,W], normalized 0-1
        pkg_mask = (batch["masks"].detach().float() > 0).unsqueeze(1)  # [B,1,Hm,Wm]
        ...
        # Downsample RGB exposure cues to GT mask size first, then combine with GT package mask.
...
def main():
    ...
    model.val(
        ...
        name="clean_xinjiang1500_best_val300_alpha_exposure",
        exist_ok=True,
    )
