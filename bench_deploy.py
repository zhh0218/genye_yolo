...
MODELS=[("早期FRM/FFM", T+"/YOLO11s-genye-ablation-old-cmx-ffm-stable/weights/best.pt"),
        ("CAF", G+"/coord_attv2-s7/weights/best.pt"),
...
for tag, w in MODELS:
    ...
    try: gflops = get_flops(m.model, imgsz=640)
    except Exception as e: gflops = -1
    ms = "NA"
    try:
        ...
        with torch.no_grad():
            for _ in range(15): net(x)
            torch.cuda.synchronize(3); ts=[]
            for _ in range(80):
                torch.cuda.synchronize(3); t0=time.perf_counter()
                net(x); torch.cuda.synchronize(3); ts.append((time.perf_counter()-t0)*1000)
        ms = "%.2f" % statistics.median(ts)
        net.to("cpu"); del net, x; torch.cuda.empty_cache()
