...
net = YOLO(GY).model
gms = [getattr(mod, "gate_mode", "?") for mod in net.modules()
...
for mod in net.modules():
    if type(mod).__name__ == "CoordAttV2" and getattr(mod, "adaptive_gate", False):
        ...
        print("  spatial mod_gate has Conv2d:", has_conv); assert has_conv, "mod_gate not spatial conv"
        break
...
net2 = YOLO(DY).model
g2 = [getattr(mod, "gate_mode", None) for mod in net2.modules()
      if type(mod).__name__ == "CoordAttV2" and getattr(mod, "adaptive_gate", False)]
