...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        imd = torch.zeros(self.args.batch,  model.yaml.get("channels", 3), *self.imgsz).to(self.device)
        file = Path(
        ...
        for _ in range(2):  # dry runs
            y = NMSModel(model, self.args)(im, imd) if self.args.nms and not coreml and not imx else model(im, imd)
        if self.args.half and onnx and self.device.type != "cpu":
