...
class YOLORGBDDataset(YOLODataset):
    def __init__(self, *args, depth_path: str = "", data: dict | None = None, task: str = "detect", **kwargs):
        self.depth_path = depth_path
        super().__init__(*args, data=data, task=task, **kwargs)
        self.depth_files = self.get_depth_files(self.depth_path)
        self.depth_ims, self.depth_hw0, self.depth_hw = [None] * self.ni, [None] * self.ni, [None] * self.ni
        self.depth_npy_files = [Path(f).with_suffix(".npy") for f in self.depth_files]

    def get_depth_files(self, depth_path: str) -> list[str]:
        if not depth_path:
            raise FileNotFoundError("Depth path is required for YOLORGBDDataset.")

        depth_files = self.get_img_files(depth_path)
        depth_groups = defaultdict(list)
        for f in depth_files:
            depth_groups[Path(f).stem].append(f)

        ambiguous = [stem for stem, files in depth_groups.items() if len(set(files)) > 1]
        if ambiguous:
            preview = ", ".join(ambiguous[:5])
            raise RuntimeError(f"Duplicate depth filename stem detected: {preview}")

        depth_queues = {stem: deque(files) for stem, files in depth_groups.items()}
        paired_depth_files = []
        keep_indices = []
        missing = []
        for idx, im_file in enumerate(self.im_files):
            stem = Path(im_file).stem
            queue = depth_queues.get(stem)
            if not queue:
                missing.append(Path(im_file).name)
                continue
            paired_depth_files.append(queue.popleft())
            keep_indices.append(idx)

        if missing:
            preview = ", ".join(missing[:5])
            LOGGER.warning(f"Skipping {len(missing)} RGB images with no depth pair: {preview}")
            self.im_files = [self.im_files[i] for i in keep_indices]
            if hasattr(self, "labels") and self.labels:
                self.labels = [self.labels[i] for i in keep_indices]

        extras = [files[0] for files in depth_queues.values() if files]
        if extras:
            preview = ", ".join(Path(f).name for f in extras[:5])
            LOGGER.warning(f"Skipping {len(extras)} unmatched depth images (no RGB pair): {preview}")

        return paired_depth_files

    def load_depth_image(self, i: int, rect_mode: bool = True) -> tuple[np.ndarray, tuple[int, int], tuple[int, int]]:
        im, f, fn = self.depth_ims[i], self.depth_files[i], self.depth_npy_files[i]
        if im is None:
            if fn.exists():
                try:
                    im = np.load(fn)
            ...
            if self.cache == "ram":
                self.depth_ims[i], self.depth_hw0[i], self.depth_hw[i] = im, (h0, w0), im.shape[:2]
                self.depth_ims[i].setflags(write=False)

            return im, (h0, w0), im.shape[:2]

        return self.depth_ims[i], self.depth_hw0[i], self.depth_hw[i]

    def get_image_and_label(self, index: int) -> dict[str, Any]:
        label = super().get_image_and_label(index)
        depth_img, _, _ = self.load_depth_image(index)
        if depth_img.shape[:2] != label["img"].shape[:2]:
            depth_img = cv2.resize(depth_img, (label["img"].shape[1], label["img"].shape[0]), interpolation=cv2.INTER_LINEAR)
            if depth_img.ndim == 2:
