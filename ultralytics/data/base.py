...
class BaseDataset(Dataset):
    ...
    def get_depth_files(self, img_path: str | list[str]) -> list[str]:
        ...
        Raises:
            FileNotFoundError: If no images are found or the path doesn't exist.
        """
        img_path = img_path.replace('images', 'depth')
        try:
            f = []  # image files
            for p in img_path if isinstance(img_path, list) else [img_path]:
                p = Path(p)  # os-agnostic
    ...
    def load_image(self, i: int, rect_mode: bool = True) -> tuple[np.ndarray, tuple[int, int], tuple[int, int]]:
        ...
        if im is None:  # not cached in RAM
            ...
            if rect_mode:  # resize long side to imgsz while maintaining aspect ratio
                ...
                if r != 1:  # if sizes are not equal 这里做的处理主要是将图片压缩到最长边为640
                    w, h = (min(math.ceil(w0 * r), self.imgsz), min(math.ceil(h0 * r), self.imgsz))
                    if w == 0 or h == 0:
                        raise ValueError(
                            f"Invalid resize target (w={w}, h={h}) for image {f}, "
                            f"orig shape=({h0},{w0}), imgsz={self.imgsz}, r={r}, im.shape={im.shape}"
                        )
                    try:
                        im = cv2.resize(im, (w, h), interpolation=cv2.INTER_LINEAR)
                    except cv2.error as e:
                        raise ValueError(
                            f"cv2.resize failed for {f}: dsize=({w},{h}), type_w={type(w)}, type_h={type(h)}, "
                            f"im.shape={im.shape}, im.dtype={im.dtype}, h0={h0}, w0={w0}, r={r}"
                        ) from e
                ...
                try:
                    ...
                    raise ValueError(
                        f"cv2.resize(square) failed for {f}: imgsz={self.imgsz}, im.shape={im.shape}"
                    ) from e
            if im.ndim == 2:
                im = im[..., None]
            ...
            if self.augment:
                ...
                self.ims[i].setflags(write=False)
