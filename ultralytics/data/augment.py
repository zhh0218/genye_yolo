...
class BaseMixTransform:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        labels = self._mix_transform(labels)  # 错误已经找到
        labels.pop("mix_labels", None)
        return labels  # yecanqing叶灿庆
...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        for i in range(3):
            ...
            depth_img = labels_patch.get("depth_img")
            h, w = labels_patch.pop("resized_shape")

            # Place img in img3
            if i == 0:  # center
                img3 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=dtype1)  # base image with 3 tiles
                if depth_img is not None:
                    depth3 = np.full((s * 3, s * 3, depth_img.shape[2]), 114, dtype=depth_img.dtype)
                h0, w0 = h, w
            ...
            if depth_img is not None:
                depth3[y1:y2, x1:x2] = depth_img[y1 - padh :, x1 - padw :]
        ...
        if labels.get("depth_img") is not None:
            final_labels["depth_img"] = depth3[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
    ...
    def _mosaic4(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        for i in range(4):
            ...
            # Place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=dtype1)  # base image with 4 tiles
                if depth_img is not None:
                    depth4 = np.full((s * 2, s * 2, depth_img.shape[2]), 114, dtype=depth_img.dtype)
            ...
            if depth_img is not None:
                depth4[y1a:y2a, x1a:x2a] = depth_img[y1b:y2b, x1b:x2b]
            padw = x1a - x1b
            padh = y1a - y1b
    ...
    def _mosaic9(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        for i in range(9):
            ...
            # Place img in img9
            if i == 0:  # center
                img9 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=dtype1)  # base image with 4 tiles
                if depth_img is not None:
                    depth9 = np.full((s * 3, s * 3, depth_img.shape[2]), 114, dtype=depth_img.dtype)
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
            ...
            if depth_img is not None:
                depth9[y1:y2, x1:x2] = depth_img[y1 - padh :, x1 - padw :]
            hp, wp = h, w  # height, width previous for next iteration
        ...
        if labels.get("depth_img") is not None:
            final_labels["depth_img"] = depth9[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        labels["img"] = (labels["img"] * r + labels2["img"] * (1 - r)).astype(dtype1)
        if "depth_img" in labels and "depth_img" in labels2:
            depth_dtype = labels["depth_img"].dtype
            labels["depth_img"] = (labels["depth_img"] * r + labels2["depth_img"] * (1 - r)).astype(depth_dtype)
...
class CutMix(BaseMixTransform):
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if "depth_img" in labels and "depth_img" in labels2:
            labels["depth_img"][y1:y2, x1:x2] = labels2["depth_img"][y1:y2, x1:x2]
...
class RandomFlip:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        depth_img = labels.get("depth_img")
        flip_vertical = self.direction == "vertical" and random.random() < self.p
        flip_horizontal = self.direction == "horizontal" and random.random() < self.p
        if flip_vertical:
            img = np.flipud(img)
            if depth_img is not None:
...
class LetterBox:
    ...
    def __call__(self, labels: dict[str, Any] | None = None, image: np.ndarray = None) -> dict[str, Any] | np.ndarray:
        ...
        depth_img = labels.get("depth_img") if image is None else None
        ...
        if c == 3:
            ...
            img = pad_img
        if depth_img is not None:
            hd, wd, cd = depth_img.shape
            if cd == 3:
                depth_img = cv2.copyMakeBorder(
                    depth_img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(self.padding_value,) * 3
                )
            else:
                pad_depth = np.full(
                    (hd + top + bottom, wd + left + right, cd), fill_value=self.padding_value, dtype=depth_img.dtype
                )
                pad_depth[top : top + hd, left : left + wd] = depth_img
                depth_img = pad_depth
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1: dict[str, Any], labels2: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        dtype1 = None
        if labels1["im_file"][-3:] == "png":
            dtype1 = np.float32
        else:
            dtype1 = np.uint8
        im_new = np.zeros(im.shape, dtype1)
        ...
        depth_result = None
        if depth is not None:
            depth_result = labels2.get("depth_img", cv2.flip(depth, 1))
            if depth_result.ndim == 2:
        ...
        labels1["img"] = im
        if depth is not None and depth_result is not None:
            depth[i] = depth_result[i]
            labels1["depth_img"] = depth
        labels1["cls"] = cls
...
class Format:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if depth_img is not None:
            ...
            labels["depth_img"] = torch.from_numpy(depth_img)
