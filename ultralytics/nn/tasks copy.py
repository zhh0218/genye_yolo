...
class RGBDScaleAwareWaveletFusion(nn.Module):
    def __init__(
        ...
        self.base_fusion = base_fusion_cls(c_in, num_heads=num_heads, kv_pool=kv_pool)
...
def parse_model(d, ch, verbose=True):
    ...
    def _build_wavelet_module(c_in, band, use_gate):
        return RGBDScaleAwareWaveletFusion(
            c_in,
            num_heads=8,
            kv_pool=10,
            base_fusion_cls=_fusion_cls,
            band=band,
            use_gate=use_gate,
            low_gain=_wavelet_low_gain,
