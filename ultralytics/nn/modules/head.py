...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, reg_max: int = 16, end2end: bool = False, ch: tuple = ()):
        ...
        Args:
            nc (int): Number of classes.
            reg_max (int): DFL channels. Set to 1 to remove DFL.
        ...
        self.reg_max = reg_max  # DFL channels (set to 1 to disable DFL)
...
class Segment(Detect):
    ...
    def __init__(
        self, nc: int = 80, nm: int = 32, npr: int = 256, reg_max: int = 16, end2end: bool = False, ch: tuple = ()
    ):
    ...
    def forward(self, x: list[torch.Tensor]) -> tuple | list[torch.Tensor]:
        ...
        if self.end2end:
            # End-to-end path with one2many/one2one dual branches
            x_detach = [xi.detach() for xi in x]
            one2one_mc = torch.cat([self.one2one_cv4[i](x_detach[i]).view(bs, self.nm, -1) for i in range(self.nl)], 2)
            one2one = [
            ...
            if self.training:
                return {
                    "one2many": (x, mc, p),
                    "one2one": (one2one, one2one_mc, p),
                }
            # Inference: use one2one branch
            y = self._inference(one2one)
            # Segment-aware postprocess: jointly select top detections and their mask coefficients
            y_perm = y.permute(0, 2, 1)  # [bs, num_anchors, 4+nc]
            mc_perm = one2one_mc.permute(0, 2, 1)  # [bs, num_anchors, nm]
            batch_size, anchors, _ = y_perm.shape
            boxes, scores = y_perm.split([4, self.nc], dim=-1)
            ...
            mc_sel = mc_perm.gather(dim=1, index=index.repeat(1, 1, self.nm))
            scores, index = scores.flatten(1).topk(min(self.max_det, anchors))
            i = torch.arange(batch_size)[..., None]
            y = torch.cat(
                [
                    boxes[i, index // self.nc],
                    scores[..., None],
                    (index % self.nc)[..., None].float(),
                    mc_sel[i, index // self.nc],
                ],
                dim=-1,
            )
            # y: [bs, max_det, 6+nm]
            return (
                ...
                if self.export
                    ...
                    (y, p),
                    {"one2many": (x, mc, p), "one2one": (one2one, one2one_mc, p)},
                )
            )
