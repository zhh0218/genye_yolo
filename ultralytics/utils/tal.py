...
class TaskAlignedAssigner(nn.Module):
    ...
    def __init__(self, topk: int = 13, num_classes: int = 80, alpha: float = 1.0, beta: float = 6.0, eps: float = 1e-9, stride=None, topk2=None):
        ...
        Args:
            ...
            stride (list, optional): List of stride values for STAL small-target-aware assignment.
            topk2 (int, optional): Secondary topk for additional filtering after initial topk selection.
        """
        super().__init__()
        self.topk = topk
        ...
        self.stal = stride is not None  # whether STAL is enabled
    ...
    def select_candidates_in_gts(self, xy_centers, gt_bboxes, mask_gt=None, eps=1e-9):
        ...
        Args:
            ...
            mask_gt (torch.Tensor, optional): Mask for valid GT boxes, shape (b, n_boxes, 1).
    ...
    def select_highest_overlaps(self, mask_pos, overlaps, n_max_boxes, align_metric=None):
        ...
        Args:
            ...
            align_metric (torch.Tensor, optional): Alignment metric, shape (b, n_max_boxes, h*w).
        ...
        # STAL: secondary topk2 filtering
        if self.topk2 != self.topk and align_metric is not None:
