...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            flops = (
                thop.profile(deepcopy(model), inputs=build_inputs((stride, stride)), verbose=False)[0] / 1e9 * 2
            )  # stride GFLOPs
            return flops * imgsz[0] / stride * imgsz[1] / stride  # imgsz GFLOPs
        except Exception:
            # Method 2: Use actual image size (required for RTDETR models)
            return (
                thop.profile(deepcopy(model), inputs=build_inputs(tuple(imgsz)), verbose=False)[0] / 1e9 * 2
            )  # imgsz GFLOPs
    except Exception:
        return 0.0
