...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        backbone_d = yaml.get("backboneD") if isinstance(yaml, dict) else None

        def build_inputs(size):
            inputs = [torch.empty((1, p.shape[1], *size), device=p.device)]
            if backbone_d:
                depth_idx = len(yaml.get("backbone", [])) + len(yaml.get("head", []))
                depth_ch = p.shape[1]
                if hasattr(model, "model") and len(model.model) > depth_idx:
                    try:
                        depth_ch = next(model.model[depth_idx].parameters()).shape[1]
                    except Exception:
                        pass
                inputs.append(torch.empty((1, depth_ch, *size), device=p.device))
            return inputs
        ...
        try:
            ...
            flops = thop.profile(deepcopy(model), inputs=build_inputs((stride, stride)), verbose=False)[0] / 1e9 * 2  # stride GFLOPs
            return flops * imgsz[0] / stride * imgsz[1] / stride  # imgsz GFLOPs
        except Exception:
            # Method 2: Use actual image size (required for RTDETR models)
            return thop.profile(deepcopy(model), inputs=build_inputs(tuple(imgsz)), verbose=False)[0] / 1e9 * 2  # imgsz GFLOPs
    except Exception:
        return 0.0
